...
const Layout = () => {
  ...
  useEffect(() => {
    ...
    if (authDetails) {
      ...
      const docRef = doc(
        database,
        `SCHOOLS/${accountDetails.school}/CLASSES/${sessionStorage.getItem(
          "class"
        )}`
      );
      onSnapshot(docRef, (doc) => {
        // SNAPSHOT KEEPS TRACK OF ANY CHANGES IN FIRESTORE
        dispatch(
          detailsReducer({
            ...doc.data(),
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      const q = query(
        ...
        where("student class", "==", userDetails.className)
      );
    ...
    if (userDetails) {
      {
        onSnapshot(doc(database, "SCHOOLS", userDetails.school), (doc) => {
          // keep track of any change in firestore and update user state
          getData();
        });
      }
    }
  ...
  useEffect(() => {
    ...
    if (userDetails && schoolInfo) {
      ...
      const newIds = userDetails.latestNoticeId.filter((id) =>
        schoolNoticeId.includes(id)
      );
        ...
        <main className="flex">
          <aside className="h-screen fixed translate-x-[-100%] lg:translate-x-0 w-[18%] bg-blue-500 p-5 flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2">
              ...
              <li className="p-3 rounded-md ">
                <a target="_blank" href="/add-student">
                  Add Student
                </a>{" "}
              </li>
              <li className="p-3 rounded-md ">
                <NavLink to={"/class/students"}>All Students</NavLink>{" "}
              </li>
              <li className="p-3 rounded-md ">
                <NavLink to="/class/profile">Class info</NavLink>{" "}
              </li>
              <li className="p-3 rounded-md ">
              ...
              <li className="p-3 rounded-md ">
                <NavLink to={"/class/attendance"}>Attendance</NavLink>{" "}
              </li>
          ...
          <section className="lg:ml-[18%] w-full min-h-screen">
            {schoolInfo && (
              <p className="p-2 bg-gray-300 text-sm">
                {`${schoolInfo.session.term} term of the ${
                  schoolInfo.session.year
                } session ends ${new Date(schoolInfo.session.endDate)}`}
              </p>
            )}
            {numberOfNewMessages > 0 && (
              <p>{`You have ${numberOfNewMessages} new message(s)`}</p>
            )}
            <Outlet />
          </section>
        </main>
