...
const School = () => {
  ...
  function sortEvent(arr) {
    // this function takes array of events and sorts them in descending order
    arr.sort((a, b) => {
  ...
  useEffect(() => {
    if (schoolInfo) {
      ...
      schoolInfo.activity &&
        schoolInfo.activity.forEach((item) => {
          let splitDate = new Date(item.startDate).toDateString().split(" "); // split date into array

          if (!years.hasOwnProperty(splitDate[3])) {
            // CHECK IF YEAR EXISTS IN OBJECT
            years[splitDate[3]] = { ...list };
          }

          let eventMonth = monthNames.filter((month) =>
            month.includes(splitDate[1])
          )[0]; // use short month and get full month
          years[splitDate[3]] = {
            ...years[splitDate[3]],
            [eventMonth]: sortEvent([...years[splitDate[3]][eventMonth], item]),
          };
  ...
  return (
    <main>
      ...
      <section>
        {activityList &&
          Object.keys(activityList).map((year) => {
            return (
              <div>
                  ...
                  if (activityList[year][month].length > 0) {
                    return (
                      <div key={month} className="my-4">
                        <h5 className="text-xl font-bold border-solid border-b-4">
                          {month}
                        </h5>

                        {activityList[year][month].map((el, index) => (
                          <div className="flex gap-4">
                            ...
                            <button
                              ...
                              onClick={() =>
                                deleteActivity(year, month, el.name)
                              }
                            >
                              delete
                            </button>
