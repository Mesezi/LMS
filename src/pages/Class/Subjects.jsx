  ...
  useEffect(()=>{
    ...
    if(allStudentsData && schoolInfo){
        (async()=>{
            ...
            const students = allStudentsData.filter(student=> student['student class'] === userDetails.className) // filter students in class
  ...
  const saveNewScores = async () =>{
    ...
    const updateRequests = subjectGrades.map(item=>{
        return (async  () =>{
            ...
            const classRef = doc(database, `SCHOOLS/${userDetails.school}/CLASSES/${userDetails.className}/${year}/${term}`);
